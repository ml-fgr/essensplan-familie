...
export default function EditRecipeClient({ id, initialName, initialIngredients, initialSteps }: Props) {
  ...
  return (
    <div style={{ paddingBottom: 40 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '20px 16px 8px' }}>
        ...
        <h1 style={{ fontSize: 24, fontWeight: 700, letterSpacing: '-0.02em', margin: 0 }}>Rezept bearbeiten</h1>
      </div>
      ...
      <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div>
            ...
            {ingredients.map((ing, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', padding: '2px 12px', gap: 8, borderBottom: i < ingredients.length - 1 ? '0.5px solid rgba(31,42,34,0.08)' : 'none' }}>
                ...
                <button
                  ...
                  style={{ background: 'none', border: 'none', color: ingredients.length === 1 ? 'var(--muted)' : 'var(--danger)', fontSize: 18, cursor: ingredients.length === 1 ? 'default' : 'pointer', padding: '4px', opacity: ingredients.length === 1 ? 0.3 : 1 }}
                >
                  ×
                </button>
              </div>
          ...
          <button className="btn-ghost" style={{ marginTop: 6, padding: '8px 0', fontSize: 14 }} onClick={addIngredient}>
            + Zutat hinzufügen
          </button>
        </div>
